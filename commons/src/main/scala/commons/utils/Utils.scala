...
object DateUtils{
	...
	val DATEKEY_FORMAT:DateTimeFormatter = DateTimeFormat.forPattern( "yyyyMMdd" )
...
object StringUtils{
	...
	def setFieldInConcatString(str:String, delimiter:String, field:String, newFieldValue:String):String ={
		...
		for( fileds <- str.split( delimiter ) ) {
			var arra:Array[String] = fileds.split( "=" )
		...
		fieldsMap.map( (item:(String, String)) => {
			item._1 + "=" + item._2
		} ).mkString( delimiter )
	}
...
object ValidUtils{
	 ...
	 * @return 校验结果
	 */
	def between(data:String,
	            dataField:String,
	            parameter:String,
	            startParamField:String,
