...
object CreateMySqlPool{
	...
	def apply():GenericObjectPool[MySqlProxy] ={
		...
		if( this.genericObjectPool == null ) {
			this.synchronized{
				// 获取MySQL配置参数
				val jdbcUrl:String = ConfigurationManager.config.getString( Constants.JDBC_URL )
				val jdbcUser:String = ConfigurationManager.config.getString( Constants.JDBC_USER )
				val jdbcPassword:String = ConfigurationManager.config.getString( Constants.JDBC_PASSWORD )
